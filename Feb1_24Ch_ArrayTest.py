#Testing 16-rung BC/24-channel array with phantom 
#slightly magnetic quad hybrid BNCs, few loops loose
import suspect
import numpy as np
import scipy.signal
...
data= suspect.io.load_twix('/Users/johnnyderhovagimian/Desktop/31PMRS_Data/31P_Data_24Channel/Feb_1/CSI_FID/meas_MID00080_FID14981_csi_fid_16rung.dat')
...
for i in range(16):
    for j in range(8):
        for k in range(16):
            SNR_map[i,j,k]= np.sum(np.real(spectrum_cc_wfp[i,j,k][495:540]))
...
fig6= plt.figure()
plt.plot(np.real(cc_wfp_filtered[7,4,7].spectrum()))
# plt.xlim([15,-15])
